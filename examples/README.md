...
- `05_merkle_log.py` — append-only Merkle log with tamper detection across
  the entire chain (not just individual receipts).
- `06_postgres_store.py` — persisting receipts to PostgreSQL.
- `07_multi_agent_call_tree.py` — using `parent_trace_id` to reconstruct
  multi-agent call trees as required by EU AI Act Article 12.
- `08_compliance_export.py` — exporting receipts in a format suitable for
  SIEM ingestion (Splunk, Datadog, Sentinel).
- `09_html_summary.py` — rendering a human-readable audit report.
